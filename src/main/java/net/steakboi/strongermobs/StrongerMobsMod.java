...
public class StrongerMobsMod implements ModInitializer {
	...
	public static ItemStack getPiglinSword(Random random, MobEntity mobEntity) {
		...
		addEnchantment(weapon, mobEntity, SHARPNESS, sharpness_level);
		if (random.nextInt(100) < fireAspectChance) addEnchantment(weapon, mobEntity, FIRE_ASPECT, 2);
		if (random.nextInt(100) < knockbackChance) addEnchantment(weapon, mobEntity, KNOCKBACK, 2);
		return weapon;
	}
	...
	public static ItemStack getSkeletonBow(Random random, MobEntity mobEntity) {
		...
		addEnchantment(bow, mobEntity, POWER, power_level);
		if (random.nextInt(100) < punchChance) addEnchantment(bow, mobEntity, PUNCH,random.nextInt(2)+1);
		if (random.nextInt(100) < flameChance) addEnchantment(bow, mobEntity, FLAME,1);
		return bow;
	}
	...
	private static void equipPiglinArmor(EquipmentSlot equipmentSlot, Random random, MobEntity mobEntity, ArmorTrim trimGold, ArmorTrim trimNetherite, World world) {
		...
		addEnchantment(armorPiece, mobEntity, PROTECTION, protection_level);

		if (trimGold != null && trimNetherite != null) {
			if (itemToArmorMaterialMap.get(armorItem) == ArmorMaterials.NETHERITE) {
	...
	public static void EquipArmor(Random random, MobEntity mobEntity){
		...
		World world = mobEntity.getEntityWorld();
		ArmorTrim armorTrim = getTrim(random, mobEntity, itemToArmorMaterialMap.get(ArmorMap.get(EquipmentSlot.FEET).get(quality)));
		...
		if (equipmentSlot.getType() == EquipmentSlot.Type.HUMANOID_ARMOR) {
			...
			if (itemStack.isEmpty()) {
				...
				if (item != null) {
					...
					addEnchantment(armor_piece, mobEntity, PROTECTION, protectionLevel);
					if (armorTrim != null) {
	...
	private static RegistryEntry<ArmorTrimPattern> getTrimPattern(Random random, World world){
		...
		Registry<ArmorTrimPattern> ArmorTrimPatternRegistry = world.getRegistryManager().getOrThrow(RegistryKeys.TRIM_PATTERN);
	...
	private static RegistryEntry<ArmorTrimMaterial> getTrimMaterial(Random random, World world, ArmorMaterial armorMaterial) {
		...
		Registry<ArmorTrimMaterial> ArmorTrimMaterialRegistry = world.getRegistryManager().getOrThrow(RegistryKeys.TRIM_MATERIAL);
	...
	private static ArmorTrim getTrim(Random random, MobEntity mobEntity, ArmorMaterial armorMaterial) {
		...
		RegistryEntry<ArmorTrimPattern> trim = getTrimPattern(random, mobEntity.getEntityWorld());
		if (trim == null){
			return null;
		}
		RegistryEntry<ArmorTrimMaterial> mat = getTrimMaterial(random, mobEntity.getEntityWorld(), armorMaterial);
	...
	private static final Map<Item, ArmorMaterial> itemToArmorMaterialMap = new HashMap<>(){{
		put(Items.NETHERITE_BOOTS, ArmorMaterials.NETHERITE);
		put(Items.NETHERITE_LEGGINGS, ArmorMaterials.NETHERITE);
		put(Items.NETHERITE_CHESTPLATE, ArmorMaterials.NETHERITE);
		put(Items.NETHERITE_HELMET, ArmorMaterials.NETHERITE);
		put(Items.DIAMOND_BOOTS, ArmorMaterials.DIAMOND);
		put(Items.DIAMOND_LEGGINGS, ArmorMaterials.DIAMOND);
		put(Items.DIAMOND_CHESTPLATE, ArmorMaterials.DIAMOND);
		put(Items.DIAMOND_HELMET, ArmorMaterials.DIAMOND);
		put(Items.IRON_BOOTS, ArmorMaterials.IRON);
		put(Items.IRON_LEGGINGS, ArmorMaterials.IRON);
		put(Items.IRON_CHESTPLATE, ArmorMaterials.IRON);
		put(Items.IRON_HELMET, ArmorMaterials.IRON);
		put(Items.GOLDEN_BOOTS, ArmorMaterials.GOLD);
		put(Items.GOLDEN_LEGGINGS, ArmorMaterials.GOLD);
		put(Items.GOLDEN_CHESTPLATE, ArmorMaterials.GOLD);
		put(Items.GOLDEN_HELMET, ArmorMaterials.GOLD);
		put(Items.CHAINMAIL_BOOTS, ArmorMaterials.CHAIN);
		put(Items.CHAINMAIL_LEGGINGS, ArmorMaterials.CHAIN);
		put(Items.CHAINMAIL_CHESTPLATE, ArmorMaterials.CHAIN);
		put(Items.CHAINMAIL_HELMET, ArmorMaterials.CHAIN);
		put(Items.LEATHER_BOOTS, ArmorMaterials.LEATHER);
		put(Items.LEATHER_LEGGINGS, ArmorMaterials.LEATHER);
		put(Items.LEATHER_CHESTPLATE, ArmorMaterials.LEATHER);
		put(Items.LEATHER_HELMET, ArmorMaterials.LEATHER);
	}};
